...
scalar changed_individual_price /5/;
...
Objective..Z=e=sum(i,teampass_price(i) * x(i)) + sum(p,sod(p)*individual_price(p)) + sum(p,sod(p)*changed_individual_flag(p)*changed_individual_price) -  sum(p,zarar(p)*individual_price(p)*2) -  sum(p,zarar(p)*changed_individual_flag(p)*changed_individual_price*2);
...
round2_q7_L..sum(i,round2_q7_i(i)* x(i)) + sum(j,2*round2_q7_j(j)* y(j)) + sum(j,2*round2_q7_j(j)* zb(j)* bigM)=l=zero;
...
round3_q4_L..sum(j,round3_q4_j(j)* y(j)) + sum(k,2*round3_q4_k(k)* s(k)) + sum(k,2*round3_q4_k(k)* tb(k)* bigM)=l=zero;


final_q1_Le..sum(k,final_q1_k(k)* s(k)) + sum(l,2*final_q1_l(l)* R(l)) + sum(l,2*final_q1_l(l)* wb(l)* bigM)=l=zero;
final_q2_Le..sum(k,final_q2_k(k)* s(k)) + sum(l,2*final_q2_l(l)* R(l)) + sum(l,2*final_q2_l(l)* wb(l)* bigM)=l=zero;

winner_Le..sum(l,winner_l(l)* R(l)) + sum(m,2*winner_m(m)* U(m)) + sum(m,2*winner_m(m)* vb(m)* bigM)=l=zero;
...
round2_q7_di_L..sum(i,round2_x13(i)* x(i)) + sum(j,round2_q7_j(j)* y(j)) + sum(j,round2_q7_j(j)* (1-zb(j))* bigM)=l=zero;
...
round3_q4_di_L..sum(j,round3_y7(j)* y(j)) + sum(k,round3_q4_k(k)* s(k)) + sum(k,round3_q4_k(k)* (1-tb(k))* bigM)=l=zero;


final_q1_di_Le..sum(k,final_s1(k)* s(k)) + sum(l,final_q1_l(l)* R(l)) + sum(l,final_q1_l(l)* (1-wb(l))* bigM)=l=zero;
final_q2_di_Le..sum(k,final_s3(k)* s(k)) + sum(l,final_q2_l(l)* R(l)) + sum(l,final_q2_l(l)* (1-wb(l))* bigM)=l=zero;

winner_di_Le..sum(l,winner_R1(l)* R(l)) + sum(m,winner_m(m)* U(m)) + sum(m,winner_m(m)* (1-vb(m))* bigM)=l=zero;
...
round2_q7_R..sum(i,round2_q7_i(i)* x(i)) + sum(j,2*round2_q7_j(j)* y(j)) - sum(j,2*round2_q7_j(j)* zb(j)* bigM)=g=zero;
...
round3_q4_R..sum(j,round3_q4_j(j)* y(j)) + sum(k,2*round3_q4_k(k)* s(k)) - sum(k,2*round3_q4_k(k)* tb(k)* bigM)=g=zero;


final_q1_RI..sum(k,final_q1_k(k)* s(k)) + sum(l,2*final_q1_l(l)* R(l)) - sum(l,2*final_q1_l(l)* wb(l)* bigM)=g=zero;
final_q2_RI..sum(k,final_q2_k(k)* s(k)) + sum(l,2*final_q2_l(l)* R(l)) - sum(l,2*final_q2_l(l)* wb(l)* bigM)=g=zero;

winner_RI..sum(l,winner_l(l)* R(l)) + sum(m,2*winner_m(m)* U(m)) - sum(m,2*winner_m(m)* vb(m)* bigM)=g=zero;
...
round2_q7_di_R..sum(i,round2_x13(i)* x(i)) + sum(j,round2_q7_j(j)* y(j)) - sum(j,round2_q7_j(j)* (1-zb(j))* bigM)=g=zero;
...
round3_q4_di_R..sum(j,round3_y7(j)* y(j)) + sum(k,round3_q4_k(k)* s(k)) - sum(k,round3_q4_k(k)* (1-tb(k))* bigM)=g=zero;


final_q1_di_RI..sum(k,final_s1(k)* s(k)) + sum(l,final_q1_l(l)* R(l)) - sum(l,final_q1_l(l)* (1-wb(l))* bigM)=g=zero;
final_q2_di_RI..sum(k,final_s3(k)* s(k)) + sum(l,final_q2_l(l)* R(l)) - sum(l,final_q2_l(l)* (1-wb(l))* bigM)=g=zero;

winner_di_RI..sum(l,winner_R1(l)* R(l)) + sum(m,winner_m(m)* U(m)) - sum(m,winner_m(m)* (1-vb(m))* bigM)=g=zero;
...
Option MIP = Cplex,optcr=0,optca=0,reslim=60;
...
loop(range,
         changed_individual_price = changed_individual_price + 1;
         Solve  games using MIP maximizing Z;
         solveStatus(range) = games.solvestat;
         modelStatus(range) = games.modelstat;
         xall(range) = changed_individual_price;
         zall(range) = z.l;
         display  changed_individual_price, z.l;
...
execute_unload 'last_outputs_c4', xall
,zall,modelStatus,solveStatus;
