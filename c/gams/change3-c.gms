...
scalar changed_teampass_price /15/;
...
Objective..Z=e=sum(i,teampass_price(i) * x(i)) + sum(i,changed_teampass_flag(i) * x(i)*changed_teampass_price) + sum(p,sod(p)*individual_price(p)) -  sum(p,zarar(p)*individual_price(p)*2);
...
loop(range,
         changed_teampass_price = changed_teampass_price + 1;
         Solve  games using MIP maximizing Z;
         solveStatus(range) = games.solvestat;
         modelStatus(range) = games.modelstat;
         xall(range) = changed_teampass_price;
         zall(range) = z.l;
         display  changed_teampass_price, z.l;
...
execute_unload 'last_outputs_c3', xall
